		...
		if( isMacOS() ) {
			try {
				...
				if(new File(baseDir + "/ReplicatorG.app/Contents/Resources").exists())
					...
					Base.logger.severe("Resource folder " + baseDir + "/ReplicatorG.app not found. Using fallback " + baseDir + "/replicatorg");
					if (new File(baseDir + "/replicatorg").exists()) {
					Base.logger.info("Resources loaded");
					} else {
					Base.logger.severe("Fallback resource folder" + baseDir + "/replicatorg not found either.");
					Base.logger.severe("Resources could not loaded.");
					}
					//Return fallback regardless of whether it exists.
